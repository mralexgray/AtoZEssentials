...
- (void)stopObserving {
	...
	for (NSString *keyPath in keyPaths)[observee removeObserver:self forKeyPath:keyPath context: &BKBlockObservationContext];
}
...
- (void)removeAllBlockObservers {
	...
	for (BKObserver *trampoline in dict.allValues)	[trampoline stopObserving];
}
...
- (void)bk_setObserverBlocks:(NSMutableDictionary *)dict {
	objc_setAssociatedObject(self, &kObserverBlocksKey, dict, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
- (NSMutableDictionary *)bk_observerBlocks { return objc_getAssociatedObject(self, &kObserverBlocksKey); }
